...
class BettorDashCard extends Component {
    constructor(props) {
        super(props);
    ...
    renderPlacedCoins(){
        let coins = [];
        let coinArray = this.props.placedCoins;

        for (let i = 0; i < coinArray.length; i++){
            coins.push(<img className="dash-logo" key={`coin-${coinArray[i]}`} alt="dash-logo" src={process.env.PUBLIC_URL + `/coin-svg/${pe.getCoinSymbol(coinArray[i]).toLowerCase()}.svg`} />)
        }
