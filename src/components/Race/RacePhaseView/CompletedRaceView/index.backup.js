  ...
  coinsDetailView(race) {
    ...
    return (<div className="standardList">
              ...
              <Card bordered={false} style={{marginTop: 24}} bodyStyle={{padding: '0 32px 40px 32px'}}>
                <List className="listCard" size="large" rowKey="id" loading={false} dataSource={coins}
                      renderItem={coin => (
                              <ListItem>
                                <ListItemMeta title=" " description={this.getCoinName(coin.coinId)} avatar={
                                  <Avatar src={`/coin-svg/${priceengine.getCoinSymbol(coin.coinId).toLowerCase()}.svg`} shape="square" size="large"/>}/>
